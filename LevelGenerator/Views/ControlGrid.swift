...
struct ControlGrid: View {
    ...
    // Helper functions for grid interaction
    ...
    private func updateValues(location: CGPoint, in size: CGSize) {
        // Calculate proportion of position in control grid (0-1)
        let proportionX = location.x / size.width
        let proportionY = location.y / size.height
        
        // Map proportion to map range
        let mapX = 16.0 + (384.0 - 16.0) * proportionX  // Range 16-384
        let mapY = 16.0 + (224.0 - 16.0) * proportionY  // Range 16-224
